...
<li class="flex items-center shadow appearance-none border rounded w-full py-2 px-3 leading-tight dark:bg-zinc-900 dark:border-zinc-500 focus:dark:bg-zinc-800 mb-1">
    ...
    {/if}
    {#if editEnabled}
    <input class="focus:shadow ml-3 appearance-none focus:border focus:rounded w-full py-2 pl-2 leading-tight focus:outline-none focus:shadow-outline dark:bg-zinc-900 dark:border-zinc-400 focus:dark:bg-zinc-800 focus:border-violet-500 dark:focus:border-violet-600" name="option_entry" id="option_entry" type="text" bind:value={option.text} on:change={() => {editOption(option.identity, option.text)}}/>
    {:else}
